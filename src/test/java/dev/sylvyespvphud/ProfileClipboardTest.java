...
class ProfileClipboardTest {
    ...
    @Test void nonDefaultValuesAcrossEverySectionRoundTripExactly(){JsonObject root=GSON.toJsonTree(ProfileSettings.defaults()).getAsJsonObject();root.getAsJsonObject("pitch").addProperty("targetPitch",12.25);root.getAsJsonObject("damage").addProperty("fallTemplate","落下 {blocks} ✓");root.getAsJsonObject("attributeSwaps").addProperty("soundEnabled",false);root.getAsJsonObject("survival").addProperty("healthText","Heal ♥ now");root.getAsJsonObject("hud").getAsJsonObject("pitch").addProperty("x",137);root.getAsJsonObject("reachOutlines").addProperty("intensity",.37);root.getAsJsonObject("vectors").addProperty("size",19);root.getAsJsonObject("playerTracker").addProperty("radius",93);root.getAsJsonObject("playerTracker").addProperty("placement","HORIZON");root.getAsJsonObject("playerTracker").addProperty("nearFadeDistance",8);ProfileSettings settings=GSON.fromJson(root,ProfileSettings.class).validated();String encoded=ProfileClipboard.encode(profile("PvP 日本語",settings));var decoded=ProfileClipboard.decode(encoded);assertTrue(encoded.length()<2000);assertEquals(settings,decoded.settings());assertEquals("PvP 日本語",decoded.name());}

    @Test void stableSchemaFieldIdsDoNotDrift(){assertEquals(132,ProfileClipboard.schemaFieldCount());assertEquals(0,ProfileClipboard.schemaFieldId("alignX"));assertEquals(15,ProfileClipboard.schemaFieldId("damage"));assertEquals(40,ProfileClipboard.schemaFieldId("healingItems"));assertEquals(93,ProfileClipboard.schemaFieldId("schemaVersion"));assertEquals(101,ProfileClipboard.schemaFieldId("sounds"));assertEquals(118,ProfileClipboard.schemaFieldId("vectors"));assertEquals(129,ProfileClipboard.schemaFieldId("y"));assertEquals(130,ProfileClipboard.schemaFieldId("placement"));assertEquals(131,ProfileClipboard.schemaFieldId("nearFadeDistance"));}

    @Test void legacySph1StillImports()throws Exception{var p=profile("Legacy",ProfileSettings.defaults());JsonObject root=GSON.toJsonTree(Map.of("schemaVersion",1,"name",p.name(),"settings",p.settings())).getAsJsonObject();JsonObject tracker=root.getAsJsonObject("settings").getAsJsonObject("playerTracker");tracker.addProperty("schemaVersion",3);tracker.remove("placement");tracker.remove("nearFadeDistance");String json=root.toString();ByteArrayOutputStream bytes=new ByteArrayOutputStream();try(GZIPOutputStream gzip=new GZIPOutputStream(bytes)){gzip.write(json.getBytes(StandardCharsets.UTF_8));}String old="SPH1:"+Base64.getUrlEncoder().withoutPadding().encodeToString(bytes.toByteArray());assertEquals(p.settings(),ProfileClipboard.decode(old).settings());}
