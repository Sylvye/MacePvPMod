...
final class HudSettingsScreen extends Screen {
    ...
    public void extractRenderState(GuiGraphicsExtractor g,int mx,int my,float dt){super.extractRenderState(g,mx,my,dt);
        ...
        for(int i=0;i<NAMES.length;i++){bounds[i]=null;if(!all&&i!=selected)continue;if(i==0)bounds[i]=HudRenderer.pitch(g,draft.get(i),MacePvPMod.CONFIG.current(),MacePvPMod.CONFIG.current().targetPitch());else if(i==1)bounds[i]=HudRenderer.textColor(g,sample(i),draft.get(i),MacePvPMod.DAMAGE_CONFIG.current().fallColors().color(12.5));else if(i==2)bounds[i]=HudRenderer.textColor(g,sample(i),draft.get(i),MacePvPMod.DAMAGE_CONFIG.current().effectiveHitColors().color(18));else bounds[i]=HudRenderer.text(g,sample(i),draft.get(i),i==4?state:0);}
