...
final class ColorScaleEditorScreen extends Screen {
    private final Screen parent; private final Consumer<ColorScale> apply; private final String label;
    ...
    ColorScaleEditorScreen(Screen parent,String label,ColorScale scale,Consumer<ColorScale> apply) {
        ...
        mode=scale.mode();flatColor=scale.flatColor();minimum=scale.minimum();maximum=scale.maximum();keys=new ArrayList<>(scale.keys());
    }
    ...
    protected void init(){invalid.clear();int w=Math.min(420,width-32);var rows=LinearLayout.vertical().spacing(5);
        ...
        number(rows,"Domain minimum",minimum,w,n->minimum=n);number(rows,"Domain maximum",maximum,w,n->maximum=n);
    ...
    private void keyRow(LinearLayout rows,int index,int w){GradientKey key=keys.get(index);var row=LinearLayout.horizontal().spacing(4);boolean endpoint=index==0||index==keys.size()-1;
        if(endpoint)row.addChild(new StringWidget((w-72)/2,20,Component.literal(fmt(minimum+(maximum-minimum)*key.position())),font));
        else {var position=new EditBox(font,0,0,(w-72)/2,20,Component.literal("Damage value"));position.setValue(fmt(minimum+(maximum-minimum)*key.position()));position.setResponder(s->{try{double n=Double.parseDouble(s),p=(n-minimum)/(maximum-minimum);if(!Double.isFinite(p)||p<=keys.get(index-1).position()||p>=keys.get(index+1).position())throw new IllegalArgumentException();keys.set(index,new GradientKey(p,keys.get(index).color()));invalid.remove(position);position.setTextColor(0xffffffff);}catch(Exception e){invalid.add(position);position.setTextColor(0xffff7777);}if(done!=null)done.active=valid();});row.addChild(position);}
    ...
    public void extractRenderState(GuiGraphicsExtractor g,int x,int y,float dt){super.extractRenderState(g,x,y,dt);g.centeredText(font,title,width/2,12,0xffffffff);
        ColorScale scale;try{scale=value().validated();}catch(Exception e){scale=null;}for(int px=0;px<previewW;px++){double v=minimum+(maximum-minimum)*px/Math.max(1,previewW-1);int c=scale==null?flatColor:scale.color(v);g.fill(previewX+px,previewY,previewX+px+1,previewY+14,0xff000000|c);}g.centeredText(font,valid()?label+" preview":"Enter an ordered, valid domain and keys",width/2,previewY+17,valid()?0xffbbbbbb:0xffff7777);}
