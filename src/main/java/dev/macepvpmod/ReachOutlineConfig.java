...
public record ReachOutlineConfig(int schemaVersion, boolean enabled, int color, double intensity, int thickness) {
    public static ReachOutlineConfig defaults() { return new ReachOutlineConfig(1, true, 0x66ccff, .22, 1); }
    public ReachOutlineConfig validated() {
        ...
        return new ReachOutlineConfig(1, enabled, Math.clamp(color, 0, 0xffffff), alpha, Math.clamp(thickness, 1, 5));
    }
