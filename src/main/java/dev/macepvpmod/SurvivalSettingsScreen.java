...
public final class SurvivalSettingsScreen extends Screen {
    ...
    private SurvivalConfig draft() {
        return new SurvivalConfig(2, enabled[0], enabled[1], texts[0], colors[0], sizes[0], x[0], y[0],
                ...
                MacePvPMod.SURVIVAL_CONFIG.current().damageIndicatorEnabled(), MacePvPMod.SURVIVAL_CONFIG.current().damageIndicatorThickness(), MacePvPMod.SURVIVAL_CONFIG.current().damageIndicatorOpacity(), MacePvPMod.SURVIVAL_CONFIG.current().damageIndicatorColor(), MacePvPMod.SURVIVAL_CONFIG.current().enabled()).validated();
    }
