...
public final class ReachOutlineSettingsScreen extends Screen {
    ...
    public ReachOutlineSettingsScreen(Screen parent) {
        super(Component.literal("SylvyesPvPHud • Reach Outlines")); this.parent = parent;
        apply(SylvyesPvPHud.REACH_OUTLINE_CONFIG.current());
    }
    @Override protected void init() {
        ...
        addRenderableWidget(Button.builder(Component.literal("Save"),b->{try{SylvyesPvPHud.REACH_OUTLINE_CONFIG.save(new ReachOutlineConfig(1,enabled,color,intensity,thickness,topFacesOnly,hardToReachMode,minimumReachableArea));onClose();}catch(IOException e){error="Couldn't save. Check config folder permissions.";}}).bounds(x,height-28,(w-8)/2,20).build());
