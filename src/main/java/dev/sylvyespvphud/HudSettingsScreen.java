...
final class HudSettingsScreen extends Screen {
    ...
    HudSettingsScreen(Screen parent,int selected){super(Component.literal("HUD"));this.parent=parent;this.selected=selected;draft=SylvyesPvPHud.HUD_CONFIG.current();}
    ...
    protected void init(){invalid.clear();
        ...
        save=addRenderableWidget(Button.builder(Component.literal("Save"),b->{try{SylvyesPvPHud.HUD_CONFIG.save(draft);onClose();}catch(IOException e){error="Could not save HUD settings.";}}).bounds(width/2-104,height-26,100,20).build());
    ...
    private String sample(int i){var d=SylvyesPvPHud.DAMAGE_CONFIG.current();var s=SylvyesPvPHud.SURVIVAL_CONFIG.current();return switch(i){case 1->DamageText.format(d.fallTemplate(),12.5,18);case 2->DamageText.format(d.hitTemplate(),12.5,18);case 3->s.retotemText();case 5->AttributeSwaps.HUD_TEXT;default->state==1?s.healthText():state==2?s.saturationText():s.combinedText();};}
    public void extractRenderState(GuiGraphicsExtractor g,int mx,int my,float dt){super.extractRenderState(g,mx,my,dt);
        ...
        for(int i=0;i<NAMES.length;i++){bounds[i]=null;if(!all&&i!=selected)continue;if(i==0)bounds[i]=HudRenderer.pitch(g,draft.get(i),SylvyesPvPHud.CONFIG.current(),SylvyesPvPHud.CONFIG.current().targetPitch());else if(i==1)bounds[i]=HudRenderer.textColor(g,sample(i),draft.get(i),SylvyesPvPHud.DAMAGE_CONFIG.current().fallColors().color(12.5));else if(i==2)bounds[i]=HudRenderer.textColor(g,sample(i),draft.get(i),SylvyesPvPHud.DAMAGE_CONFIG.current().effectiveHitColors().color(18));else bounds[i]=HudRenderer.text(g,sample(i),draft.get(i),i==4?state:0);}
