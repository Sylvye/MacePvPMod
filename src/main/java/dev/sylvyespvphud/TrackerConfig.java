...
public record TrackerConfig(int schemaVersion, boolean enabled, int radius, double opacity, int iconSize,
                            ...
                            double hideStartDistance, int maxVisiblePlayers, TrackerPlacement placement,
                            double nearFadeDistance) {
    public static TrackerConfig defaults() {
        return new TrackerConfig(4,true,48,.9,12,true,TrackerDisplayMode.HEADS,false,.25,false,false,1000,0,
                TrackerPlacement.RING,20);
    }
    public TrackerConfig validated() {
        if(schemaVersion!=3&&schemaVersion!=4)throw new IllegalArgumentException("Unsupported Player Tracker configuration version: "+schemaVersion);
        return new TrackerConfig(4,enabled,Math.clamp(radius,16,160),HudStyle.limit(opacity,.05,1,.9),
                ...
                schemaVersion==3||placement==null?TrackerPlacement.RING:placement,
                schemaVersion==3?20:HudStyle.limit(nearFadeDistance,1,256,20));
    }
}
