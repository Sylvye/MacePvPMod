...
public final class ClientSmokeTest implements FabricClientGameTest {
    ...
    public void runTest(ClientGameTestContext context){
        ...
        context.runOnClient(mc->{
            ...
            var info=new net.minecraft.client.input.MouseButtonInfo(0,0);var press=new net.minecraft.client.input.MouseButtonEvent(screen.screenX(bounds.x()+2),screen.screenY(bounds.y()+2),info);
            ...
            check(!screen.currentStyle().equals(saved),"Unified HUD draft did not move");check(SylvyesPvPHud.HUD_CONFIG.current().fall().equals(saved),"Unified HUD drag leaked before Apply");
        });click(context,"Back to HUD Studio");click(context,"Apply");context.runOnClient(mc->check(!SylvyesPvPHud.HUD_CONFIG.current().fall().equals(HudConfig.defaults().fall()),"Unified HUD drag was not applied"));
        ...
        context.runOnClient(mc->{try{SylvyesPvPHud.HUD_CONFIG.save(HudConfig.defaults());}catch(Exception e){throw new RuntimeException(e);}});
        context.setScreen(()->new PitchSettingsScreen(null));click(context,"Enabled: On");context.runOnClient(mc->check(SylvyesPvPHud.CONFIG.current().enabled(),"Draft leaked"));click(context,"Cancel");
        context.setScreen(()->new PitchSettingsScreen(null));click(context,"Advanced settings");click(context,"Third person: Off");click(context,"Save");context.runOnClient(mc->check(SylvyesPvPHud.CONFIG.current().thirdPerson(),"Behavior save failed"));
        ...
        click(context,"Damage: Reported");click(context,"Save");context.runOnClient(mc->{check(SylvyesPvPHud.DAMAGE_CONFIG.current().calculatedDamage(),"Mode not saved");check(SylvyesPvPHud.DAMAGE_CONFIG.current().fallTemplate().equals("Fall {blocks}"),"Template not saved");});
        context.setScreen(()->new DamageSettingsScreen(null));click(context,"Fall colors: Flat");click(context,"Mode: Flat");click(context,"Add color key");click(context,"Remove");click(context,"Use colors");
        context.runOnClient(mc->check(button(mc,"Fall colors: Gradient")!=null,"Gradient draft not returned"));click(context,"Hit colors: Gradient");click(context,"Mode: Gradient");click(context,"Cancel");
        context.runOnClient(mc->check(button(mc,"Hit colors: Gradient")!=null,"Canceled color edit leaked"));click(context,"Save");context.runOnClient(mc->check(SylvyesPvPHud.DAMAGE_CONFIG.current().fallColors().mode()==ColorMode.GRADIENT,"Gradient mode not saved"));
        context.setScreen(()->new HudSettingsScreen(null,0));
        context.runOnClient(mc->{var box=descendants(mc.gui.screen()).filter(EditBox.class::isInstance).map(EditBox.class::cast).findFirst().orElseThrow();box.setValue("oops");check(!button(mc,"Save").active,"Invalid position accepted");box.setValue("25");check(SylvyesPvPHud.HUD_CONFIG.current().pitch().x()==0,"HUD draft leaked");});click(context,"Cancel");
        ...
        context.runOnClient(mc->{
            var screen=mc.gui.screen();var original=SylvyesPvPHud.HUD_CONFIG.current().fall();
            ...
            check(SylvyesPvPHud.HUD_CONFIG.current().fall().equals(original),"Dragging leaked draft");
        });click(context,"Back to controls");click(context,"Save");
        context.runOnClient(mc->{check(SylvyesPvPHud.HUD_CONFIG.current().fall().x()==30,"Drag x not saved");check(SylvyesPvPHud.HUD_CONFIG.current().fall().y()==34,"Drag y not saved");});
        context.setScreen(()->new HudSettingsScreen(null,1));click(context,"Drag / resize preview");context.waitTick();
        context.runOnClient(mc->{
            var screen=mc.gui.screen();var bounds=HudRenderer.textBounds("Fall 12.5",SylvyesPvPHud.HUD_CONFIG.current().fall(),screen.width,screen.height);
        ...
        });click(context,"Back to controls");click(context,"Save");context.runOnClient(mc->check(SylvyesPvPHud.HUD_CONFIG.current().fall().scale()==1.5,"Resize not saved"));
        for(int scale:new int[]{1,2,3}){
            ...
            context.runOnClient(mc->{var screen=mc.gui.screen();screen.keyPressed(new net.minecraft.client.input.KeyEvent(262,0,0));});click(context,"Back to controls");click(context,"Save");context.runOnClient(mc->check(SylvyesPvPHud.HUD_CONFIG.current().fall().x()>0,"Nudge not saved"));
