...
public final class ClientSmokeTest implements FabricClientGameTest {
    ...
    public void runTest(ClientGameTestContext context){
        ...
        context.runOnClient(mc->{
            var screen=(HudPlacementScreen)mc.gui.screen();var saved=MacePvPMod.HUD_CONFIG.current().fall();var bounds=screen.currentBounds();check(bounds!=null,"Unified HUD canvas did not render");
        ...
        context.runOnClient(mc->check(button(mc,"Hit colors: Gradient")!=null,"Canceled color edit leaked"));click(context,"Save");context.runOnClient(mc->check(MacePvPMod.DAMAGE_CONFIG.current().fallColors().mode()==ColorMode.GRADIENT,"Gradient mode not saved"));
